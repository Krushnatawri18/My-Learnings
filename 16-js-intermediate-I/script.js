...
let slicedCars = cars.slice(1);  // omitting start with (undefined, 3), and omitting end with (0)
...
// like here "10" < "4", cause "1" comes before "4"
...
const ar = [12, 24, 36]
const foundArrayElement = ar.find(function (val) {
    return val === 18; 
})
// console.log(foundArrayElement) // undefined
...
// 16. indexOf - returns the first index of particular value else -1 if not found
// takes two params
// 1. element - value to search for
...
// with objects directly forEach doesn't work as its not array so not iterable
...
// if writable is true and configurable is false still you can change the value of key by redefining and one time change for writable
object['webdev'] = 'React';
delete object.webdev; // deletion - no effect

// redefining won't be allowed even if writable is true
Object.defineProperty(object, 'webdev', { value: 'React' });  // works as we are just changing the value of key not any flags
...
Object.defineProperty(object, 'webdev', { writable: false });  // allowed as configurable false allows one-time change
// Object.defineProperty(object, 'webdev', { writable: true });  // error: cannot redefine the property
...
// (name) as its global variable in window.name property -> "" -> 0
// NaN - 0 = NaN
