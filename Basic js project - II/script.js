...
form.addEventListener('submit', function (e) {
    ...
    let selected = false;
    categoryRadios.forEach(function (val) {
        if (val.checked) {
            selected = val.value;
        }
    })
...
function updateStack() {
    ...
    // to update stack with three cards
    for (let i=0; i<3; i++) {
...
upBtn.addEventListener('click', function () {
    ...
    if (lastChild) {
        // will bring last child over first child of stack
...
downBtn.addEventListener('click', function () {
    ...
    if (firstChild) {
        // will append firstChild at last of stack
